...
parser.add_argument('-s', action='store', dest='s', required=True,
...
# load singletons
singletons = list()
with open(par.s, mode='r', encoding='utf-8') as f:
    for line in f:
...
for family in nx.weakly_connected_components(G):
    ...
    for node in [n for n in E.nodes if n != 'VIRTUAL']:
        ...
        n = {'form': form, 'pos': pos}
        obli = [(edge, rules[edge]) for edge in E.edges
                if edge[1] == node and edge[0] != 'VIRTUAL']
        facu = [(edge, rules[edge]) for edge in F.edges
                if edge[1] == node and edge[0] != 'VIRTUAL']
        if obli:
            n['parent'] = obli[0]
            n['ref_roots'] = ''
