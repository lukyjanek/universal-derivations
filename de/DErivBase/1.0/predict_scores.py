...
if par.r:
    ...
    with open(par.r, mode='r', encoding='U8') as f:
        for line in f:
            ...
            if len(parent.split('_')[1]) == 2:
                par_gend = parent.split('_')[1][-1]
                custom_features[(parent, child)]['parentGend'] = par_gend
            else:
                custom_features[(parent, child)]['parentGend'] = np.nan
            if len(child.split('_')[1]) == 2:
                chi_gend = child.split('_')[1][-1]
                custom_features[(parent, child)]['childGend'] = chi_gend
            else:
                custom_features[(parent, child)]['childGend'] = np.nan
            custom_features[(parent, child)]['rule'] = rule
...
with open(par.a, mode='r', encoding='U8') as f:
    for line in f:
        ...
        features = make_vector(parent=parent[0], parent_pos=parent[1][0],
                               child=child[0], child_pos=child[1][0],
                               custom=cust_feat)
...
for item in annot_data:
    ...
    if isinstance(item['parentGend'], str):
        parent += item['parentGend']
    child = item['child'] + '_' + item['childPos']
    if isinstance(item['childGend'], str):
        child += item['childGend']
...
if par.fsmi or par.fsce:
    ...
    if par.fsmi:
        ...
        addition = ['childPos', 'parentPos', 'rule', 'childGend', 'parentGend']
