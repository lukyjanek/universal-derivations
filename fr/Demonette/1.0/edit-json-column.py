...
with open(sys.argv[1], mode='r', encoding='U8') as f:
    for line in f:
        ...
        if len(line) > 1:
            ...
            if original.get('in_subparadigm_with'):
                keys = [lexeme_id[lex]
                        for lex in original['in_subparadigm_with'].split('&')]
                original['in_subparadigm_with'] = ','.join(keys)
            if original.get('other_parents'):
                keys = [lexeme_id[lex.split('&')[0]] + '&' + '&'.join(lex.split('&')[1:])
