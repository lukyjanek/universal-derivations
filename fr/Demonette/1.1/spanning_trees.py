...
with open(par.r, mode='r', encoding='utf-8') as f:
    for line in f:
        ...
        par_seg = line[4]
        chi_seg = line[5]
        par_oth = line[6].split('|')
        chi_oth = line[7].split('|')
        par_dil = line[8].split('|')
        chi_dil = line[9].split('|')
        semantics[(parent, child)] = par_sem + '#' + chi_sem
        [indirects[parent].add(i) for i in par_oth]
        [indirects[child].add(i) for i in chi_oth]
        [directless[parent].add(i) for i in par_dil]
        [directless[child].add(i) for i in chi_dil]
...
for family in nx.weakly_connected_components(G):
    ...
    for node in [n for n in E.nodes if n != 'VIRTUAL']:
        ...
        n = {'form': form, 'pos': pos, 'seg': segmentat[node],
             'inparadigm': set(list(indirects[node]) + list(directless[node]))}
